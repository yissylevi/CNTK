...
def test_session_sanity_check(tmpdir, device_id):
    ...
    c = SessionConfig(mb_source=mbs, var_to_stream=input_map,
                              mb_size=4)
...
def test_session_cross_validation_at_end(tmpdir, device_id):
    ...
    c = SessionConfig(mb_source=mbs, mb_size=4,
                      var_to_stream=input_map, max_samples=20) \
...
def test_session_cross_validation_3_times(tmpdir, device_id):
    ...
    c = SessionConfig(mb_source=mbs, mb_size=4,
                      ...
                      max_samples=60) \
        .cross_validation(source=mbs1, frequency=20, mb_size=2) \
        .progress_printing(writer)
    training_session(trainer=t, config=c).train(device)
...
def test_session_progress_print(tmpdir, device_id):
    ...
    c = SessionConfig(mb_source=mbs, mb_size=minibatch_size_schedule(4),
                      var_to_stream=input_map, max_samples=60) \
